...
use crate::engine::{loader::{gltf::{GLtypes, Gltf}, imageasset::{ImageAsset, fileopen}, jsonparser::JsonF}, math::{vec2::Vec2, vec3::Vec3, vec4::Vec4}};
...
impl Glscene{
  ...
  pub async fn read_gltf_json(path: &str) -> Glscene{
    ...
    let jgltf = JsonF::load_from_file(path).await;
    ...
    for i in 0..pgltf.buffers.len(){
      rwbf.push(fileopen(&format!("{}{}", prefix, pgltf.buffers[i].uri.clone())).await);
    }
  ...
  pub async fn readglb(path: &str) -> Glscene{
    let rglb = fileopen(path).await;
  ...
  pub async fn is_glb(path: &str) -> bool{
    let rglb = fileopen(path).await;
