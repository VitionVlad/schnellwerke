...
impl Scene{
    ...
    pub async fn load_from_gltf(eng: &mut Engine, path: &str, material: Material) -> Scene{
        ...
        if Glscene::is_glb(path).await{
            gltfsc = Glscene::readglb(path).await;
            ...
            gltfsc = Glscene::read_gltf_json(path).await;
            ...
            for i in 0..gltfsc.material_uri.len(){
              ...
              ldmt.push(Image::new_from_files(&eng, totdata).await);
            }
        }
