...
fn main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    if (mi.addinfo.z > 0.0 && mi.addinfo.z < 10.0) {
        outColor = vec4<f32>(
            1.0 - textureSample(texSampler, colorSampler, vec2<f32>(uv.x / mi.addinfo.x + (1.0 / mi.addinfo.x) * mi.addinfo.y, -uv.y), 0).rgb,
            1.0
        );
    } else if(mi.addinfo.z > 10.0){
        outColor = vec4<f32>(
            textureSample(texSampler, colorSampler, vec2<f32>(uv.x / mi.addinfo.x + (1.0 / mi.addinfo.x) * mi.addinfo.y, -uv.y), 0).rgb / 16.0,
            1.0
        );
    }else {
        outColor = vec4<f32>(
            textureSample(texSampler, colorSampler, vec2<f32>(uv.x / mi.addinfo.x + (1.0 / mi.addinfo.x) * mi.addinfo.y, -uv.y), 0).rgb,
            1.0
        );
    }
    return outColor;
