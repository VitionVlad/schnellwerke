...
fn main(
    ...
    output.fuv = uv;
    output.fpos = mi.t * mi.r * mi.s * vec4f(pos.x, pos.y, pos.z, 1.0);
    output.ftg = tg;
    output.fctg = ctg;
    output.fnormal = (mi.r * vec4f(normal, 1.0)).xyz;
    output.position = dmi.defferedView * mi.t * mi.r * mi.s * vec4f(pos.x, pos.y, pos.z, 1.0);
    return output;
}