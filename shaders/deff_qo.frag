...
fn main(
    ...
    output.outColor = vec4f(textureSample(texSampler, colorSampler, vec2f(uv.x, -uv.y), 0).rgb, 1.0);
    output.outMaterial = vec4f(1.0, 0.1, 0.1, 1.0);
    output.outNormal = vec4f(fnormal, 1.0);
    output.outPos = pos;
    return output;
}